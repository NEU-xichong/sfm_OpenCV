...
namespace FeatureGraph
{
    ...
    */
    ...
    bool Image::ExistImgId(const int id) const {
    ...
    cv::Mat Image::GetImgById(const int id) const {

        return ImgVec[id].img_;
    }

    std::vector<cv::KeyPoint> Image::GetKeypotsById(const int id) const {

        return ImgVec[id].keyPoints;
    }

    cv::Mat Image::GetDesById(const int id) const {

        return ImgVec[id].descriptor;
    }

    int Image::createVocabSim(const std::string &vocabPath,const std::vector<Img> imgVec,
                               const size_t numSim) {
     ...
     */
    void Image::Homography_Fundamental_geometric_verif() {
    ...
    int Image::createResMatch(const std::map<int, std::vector<int>> simGrap,
